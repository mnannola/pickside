...
export default async function VotePage({ params, searchParams }: Props) {
  ...
  return <div className="page-shell matchup-page"><section className="matchup-stage" aria-label="Choose your side"><div className="page-heading centered"><span className="eyebrow">{matchup.category || 'THE MOMENT OF TRUTH'}</span><h1><span className="desktop-lead">Go on. </span><span className="accent-word">Pick a side.</span></h1><p>{total === 0 ? 'A clean slate. Make the first pick.' : total + (total === 1 ? ' person has' : ' people have') + ' picked a side.'}</p></div><VotePanel matchup={matchup}/></section>{created && <div className="success-banner" role="status">✓ Your matchup is ready. Copy the link and start the debate.</div>}<div className="below-matchup"><span>No signup needed. Your pick stays anonymous.</span><Link href={'/m/' + slug + '/results'} className="text-link">Just see the results →</Link></div><Share slug={slug}/></div>;
}
