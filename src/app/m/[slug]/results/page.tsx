...
export default async function Results({ params }: Props) {
  ...
  const hash = await voterHash(); const pick = hash || creator ? await existingVote(slug, hash || '', creator?.id) : undefined; const total = matchup.a_votes + matchup.b_votes; const [a, b] = percentages(matchup.a_votes, matchup.b_votes);
