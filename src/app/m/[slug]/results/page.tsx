...
export default async function Results({ params }: Props) {
  ...
  return <div className="page-shell matchup-page"><section className="matchup-stage" aria-label="Matchup results"><div className="page-heading centered"><span className="eyebrow">{pick ? '✓ YOUR PICK IS IN' : 'WHERE EVERYONE STANDS'}</span><h1>{headline}</h1><p>{total === 0 ? 'No votes yet. Make the first pick.' : total === 1 ? 'The first vote is in. Keep it going.' : total + ' opinions. One friendly debate.'}</p></div><div className="matchup-grid"><ChoiceCard choice={matchup.a} side="A" category={matchup.category} percentage={a} votes={matchup.a_votes} selected={pick === 'A'}/><span className="vs" aria-hidden="true">VS</span><ChoiceCard choice={matchup.b} side="B" category={matchup.category} percentage={b} votes={matchup.b_votes} selected={pick === 'B'}/></div></section><div className="results-summary" data-testid="results-summary"><strong>{total} {total === 1 ? 'vote' : 'votes'}</strong><span>{total === 0 ? 'The next move is yours.' : a === b ? 'It’s a tie. Call in the group chat.' : (a > b ? matchup.a.title : matchup.b.title) + ' is out in front.'}</span></div>{!pick && <div className="centered"><Link className="button button-outline" href={'/m/' + slug}>Make your pick →</Link></div>}<Share slug={slug}/><div className="next-debate"><p>Got an even tougher choice?</p><Link href="/create" className="text-link">Create your own matchup ↗</Link></div></div>;
}
